...
namespace Ecommerce.WebUI.Areas.Admin.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        public async Task<IActionResult> CreateProduct(CreateProductDto createProductDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7220/api/Products", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync("https://localhost:7220/api/Products?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "Product", new { area = "Admin" });
            }
        ...
        public async Task<IActionResult> UpdateProduct(string id)
        {
            ...
            ViewBag.v1 = "Products";
            ViewBag.v2 = "Update Product";
            ViewBag.v3 = "Product Operations";
            var client1 = _httpClientFactory.CreateClient();
            var responseMessage1 = await client1.GetAsync("https://localhost:7220/api/Categories");
            ...
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7220/api/Products/" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateCategory(UpdateProductDto updateProductDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7220/api/Products/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "Product", new { area = "Admin" });
            }
